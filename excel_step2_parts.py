...
def _build_part_to_name_spec_from_rows(rows):
    """
    SELECT 결과(rows)에서 품번(AC)만 매칭하여
    품번 -> (파트명 AB, 규격 AK) 딕셔너리 생성. 같은 품번 여러 행이면 첫 행 기준.
    """
    result = {}
    for part_val, part_name_val, spec_val in rows:
        part_norm = str(part_val).strip() if part_val is not None else ""
        if not part_norm or part_norm in result:
            continue
        result[part_norm] = (part_name_val, spec_val)
    return result


def load_master_dicts_from_db():
    """master.db에서 Step 2용 sn_part_to_date, part_to_name_spec 딕셔너리 생성."""
    ...
    try:
        ...
        # (S/N, 품번) → 오늘 이전 가장 최근 작업일자
        cur.execute(
            """
            SELECT sn, part_no, work_date
            FROM master_alarm
        ...
        rows = cur.fetchall()
        sn_part_to_date = _build_sn_part_to_latest_date_from_rows(rows, today)

        # 품번 → (파트명, 규격)
        cur.execute(
            """
            SELECT part_no, part_name, spec
            FROM master_alarm
        ...
        rows2 = cur.fetchall()
        part_to_name_spec = _build_part_to_name_spec_from_rows(rows2)

        return sn_part_to_date, part_to_name_spec
    finally:
        conn.close()
...
def run_step2():
    ...
    sn_part_to_date, part_to_name_spec = load_master_dicts_from_db()
